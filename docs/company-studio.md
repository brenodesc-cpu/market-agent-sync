...
O executor incremental e recuperável das missões também está disponível pela API autenticada e pelo MCP local. A validação passou com 100 testes de aplicação, 26 testes PostgreSQL, TypeScript e build. Ela cobre briefing em até três rodadas, limite de cinco etapas, orçamento por etapa, concorrência, criação e contratação atômicas, lease, retomada, revisão pendente, reconciliação após aceite, liquidação única, bloqueio de correções com descendentes antigos e o protocolo MCP por `stdio`.

O pacote público conferido em 19/09 já contém o Agente Zero, o briefing e a cadeia. O banco remoto ainda precisa de uma validação autenticada que prove a presença da migração `0012_persist_autonomous_missions.sql`. O bootstrap agora consulta essa tabela e bloqueia o início da missão quando a persistência estiver ausente. A correção de navegação que volta do aceite para a mesma missão também espera a próxima publicação.
