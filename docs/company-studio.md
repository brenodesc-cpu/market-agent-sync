...
Em `/studio?view=mission`, descreva uma meta e autorize um orçamento. O Agente Zero usa a NeuraLake para conferir objetivo, público, entrega e restrições. Ele faz até duas rodadas de perguntas, preserva as respostas e consolida o briefing antes de gastar créditos. Em seguida, monta uma cadeia de uma a cinco etapas. Em cada etapa, a plataforma pode usar o agente da própria empresa, comparar propostas de especialistas do marketplace ou criar um especialista sob demanda. As chamadas são incrementais: planejamento e execução avançam uma unidade por vez, e o estado salvo permite continuar depois de recarregar a página. Se o plano da IA vier fora do contrato JSON, um planejador local escolhe uma oferta compatível ou cria o especialista para a missão continuar.
...
O executor incremental e recuperável das missões também está disponível pela API autenticada e pelo MCP local. A validação passou com 103 testes de aplicação, 26 testes PostgreSQL, TypeScript e build. Ela cobre briefing em até duas rodadas, limite de cinco etapas, plano de contingência, orçamento por etapa, concorrência, criação e contratação atômicas, lease, retomada, revisão pendente, reconciliação após aceite, liquidação única, bloqueio de correções com descendentes antigos e o protocolo MCP por `stdio`.

A migração `0012_persist_autonomous_missions.sql` foi aplicada no banco conectado ao Lovable. Em 19/09, a prévia do commit `d209697` completou uma missão autenticada com a empresa Jarvis: o fallback escolheu Propostas Studio dentro do teto de 30 créditos, a primeira entrega falhou porque trouxe uma de quatro seções e o pagamento permaneceu bloqueado. A correção entregou Diagnóstico, Escopo, Cronograma e Investimento; todos os testes passaram, o responsável aceitou a versão 2 e 15 créditos simulados foram liquidados uma vez. A cadeia reconciliou o contrato e terminou como `completed`. O pedido tem prefixo `badad004`. A mesma versão foi publicada em `market-agent-sync.lovable.app` depois da validação.
...
O Google gerenciado foi ativado após autorização explícita em 19/09/2026. O login real completou o redirecionamento com `openid`, `email` e `profile`, restaurou a sessão e carregou as empresas da conta. O e-mail permanece como alternativa. O ambiente local continua sem broker OAuth e deve encaminhar o usuário ao endereço publicado.

O cliente agora impede a navegação local ao 404, preserva o rascunho, explica a limitação e apresenta um link para o domínio público. O rascunho não muda de domínio automaticamente. A volta do Google usa o estúdio e apresenta os erros do callback. A sessão só é aceita depois de conferir o resultado de `setSession`, incluindo erros retornados sem exceção. Seis testes de regressão cobrem esses casos.
