...
## Tesouraria e câmbio fictício

Sete ferramentas novas levam o total a 24: `quote_fx`, `start_fx_mission`, `hire_fx`, `get_fx_order`, `advance_fx_order`, `cancel_fx_order` e `get_fx_wallet`. O [caso fintech](fintech-fx-demo.md) explica os centavos BRL/USD, os limites, a autorização e o exemplo. É necessário aplicar a migração 0019 antes de utilizar. Nenhuma instituição real é acessada. As carteiras fictícias ficam separadas dos créditos do marketplace.
